...
class SignalEmitter:
    ...
    def fdisk_signal(self, path="/", sensor=(0, 0)) -> SuprSignal:
        try:
            usage = psutil.disk_usage(path)
            percent_used = usage.percent
            color = self.get_color(percent_used)
        except:
            color = self.get_color_unavail()
    ...
    def bt_signal(self, sensor=(0, 0)):
        ...
        return SuprSignal(sensor, self.get_color_by_switch(props.Get("org.bluez.Adapter1", "Powered")))

    def term_signal(self, zone=0, sensor=(0,0)):
        try:
            with open(f"/sys/class/thermal/thermal_zone{zone}/temp", "r") as f:
                milligradi = int(f.read().strip())
                gradi = milligradi / 1000.0
                return SuprSignal(sensor, self.get_color(round(gradi, 1) ))
